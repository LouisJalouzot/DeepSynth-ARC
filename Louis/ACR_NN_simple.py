...
class Net(nn.Module):
    def __init__(self, primitive_types, nb_inputs_max=6, nb_objects_max=10):
        ...
        self.nb_channels = self.nb_inputs_max * 2 * (self.nb_objects_max + 10)
        self.net = nn.Sequential( # (240, 30, 30)
            nn.Conv2d(self.nb_channels, self.nb_channels // 4, 3), # (60, 28, 28)
            nn.Conv2d(self.nb_channels // 4, self.nb_channels // 8, 3), # (30, 26, 26)
            nn.MaxPool2d(2), # (30, 13, 13)
            nn.Flatten(), # (5070)
            nn.Linear(self.nb_channels // 8 * 13 * 13, 2048),
            nn.Linear(2048, 512),
            nn.Linear(512, self.nb_primitives),
        )
        ...
        for index, obj in enumerate(list_objects):
            ...
            for i, j, c in obj.points:
                if 0 <= i + obj.low[0] < 30 and 0 <= j + obj.low[1] < 30 and 0 <= c <= 9:
                    colors[c, i + obj.low[0], j + obj.low[1]] = 1
                    objects[index, i + obj.low[0], j + obj.low[1]] = 1
        return torch.cat([colors, objects]) # (10 + nb_objects_max, 30, 30)

    def embed_task(self, pb):
        res = torch.zeros(self.nb_inputs_max, 2, self.nb_objects_max + 10,  30, 30)
        for index, pair in enumerate(pb['train']):
            if index >= self.nb_inputs_max: break
            res[index][0] = self.embed_list_obj(pair['input'][0])
            res[index][1] = self.embed_list_obj(pair['output'])
        return torch.reshape(res, (self.nb_channels, 30, 30)) # (nb_channels, 30, 30)
